...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></div></div><div class="space-y-4"><h2 class="text-2xl font-bold text-blue-600">Dur\xE9e souhait\xE9e</h2><div class="grid grid-cols-2 md:grid-cols-4 gap-4"><!--[-->`);
      ssrRenderList(durees, (d) => {
        _push(`<button type="button" class="${ssrRenderClass([
          "p-4 rounded-2xl border-2 transition-all text-center cursor-pointer",
          formData.value.duree === d.id ? "border-blue-600 bg-blue-50 shadow-lg" : "border-gray-300 hover:border-blue-400"
        ])}"><div class="text-lg mb-1">${ssrInterpolate(d.label)}</div></button>`);
      });
      _push(`<!--]--></div></div><div class="flex gap-4"><button type="submit"${ssrIncludeBooleanAttr(isLoading.value) ? " disabled" : ""} class="flex-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white py-4 rounded-2xl hover:from-blue-600 hover:to-blue-700 transition-all font-bold text-lg disabled:opacity-50">${ssrInterpolate(isLoading.value ? "\u23F3 En cours..." : "\u2705 R\xC9SERVER")}</button>`);
      ...
      if (successMessage.value) {
        _push(`<div class="bg-green-100 border-2 border-green-600 text-green-700 p-4 rounded-xl">${ssrInterpolate(successMessage.value)}</div>`);
      } else {
        _push(`<!---->`);
      }
...
//# sourceMappingURL=reservation-DiK-xhMt.mjs.map
